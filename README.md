...
- 声音分享模式(点分享时选择、共享中可切换):
  - 混合(屏幕+麦克风)
  - 仅屏幕声音(手机端为系统内音采集 AudioPlaybackCapture,需 Android 10+;共享期间物理麦克风被关闭,不采集麦克风内容)
  - 仅麦克风
  - 无声
...
- **选"仅屏幕声音"还是弹"录制音频"权限**:安卓系统规定声音内录(AudioPlaybackCapture)必须持有"录制音频"(RECORD_AUDIO)运行时权限,弹窗无法省去;但该模式下应用会在共享开始后把 WebRTC 采集源换成系统内录并立即停掉物理麦克风,麦克风内容不会被采集或发送。
- **屏幕内声音没声音**:确认已授予"录制音频"权限(所有声音模式都需要);正在播放的应用若禁止采集(如部分音乐/DRM 应用)则采不到;共享中可切换模式,无需重新发起。
